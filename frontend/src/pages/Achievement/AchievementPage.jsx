...
const AchievementsTable = () => {
  ...
  const filteredAchievements = filterType 
    ? mockAchievements.filter(achievement => achievement.achievement_type === filterType)
    : mockAchievements;

  const getLevelProgress = () => {
    const totalPointsForNextLevel = summary.total_points + summary.points_to_next_level;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10">
            <div className="text-2xl font-bold text-white">{summary.total_points}</div>
            <div className="text-sm text-gray-400">Total Points</div>
          </div>
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10">
            <div className="text-2xl font-bold text-blue-400">Level {summary.current_level}</div>
            <div className="text-sm text-gray-400">Current Level</div>
          </div>
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10">
            <div className="text-2xl font-bold text-purple-400">{summary.points_to_next_level}</div>
            <div className="text-sm text-gray-400">Points to Next Level</div>
          </div>
          <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10">
            <div className="text-2xl font-bold text-green-400">{summary.total_achievements}</div>
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-xl p-6 border border-white/10">
          <div className="flex justify-between items-center mb-2">
            <span className="text-white font-medium">Level {summary.current_level} Progress</span>
            <span className="text-gray-400 text-sm">
              {summary.total_points} / {summary.total_points + summary.points_to_next_level} points
            </span>
          </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="text-white text-xs">
                        {getMetadataDisplay(achievement.metadata, achievement.achievement_type)}
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="text-white text-xs">
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-xl p-4 border border-white/10">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-400">
              Showing 1 to {Math.min(perPage, filteredAchievements.length)} of {filteredAchievements.length} achievements
            </div>
